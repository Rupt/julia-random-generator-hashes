...
@testset "BitSift.xor_product" begin
    identity = XorMatrix64([UInt64(1) << i for i in 0:63])
    matrix = XorMatrix64([query(SplitMix(), i) for i in UInt64.(1:64)])
    matrix_alt = XorMatrix64([query(SplitMix(), i) for i in UInt64.(1001:1064)])
    zero_matrix = XorMatrix64(zeros(UInt64, 64))
